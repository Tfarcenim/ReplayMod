...
import static com.replaymod.render.blend.Util.getBlockEntityId;

public class BlockEntityExporter implements Exporter {
    ...
    public BlockEntityExporter(RenderStateShard renderState) {
    ...
    public void preTileEntitiesRender() {
        ...
        Matrix4f.translate(new Vector3f(
                (float) -mc.getEntityRenderDispatcher().camera.getPosition().x,
                (float) -mc.getEntityRenderDispatcher().camera.getPosition().y,
                (float) -mc.getEntityRenderDispatcher().camera.getPosition().z
        ), modelView, modelView);
    ...
    public void preRender(BlockEntity tileEntity, double dx, double dy, double dz, float renderPartialTicks, int destroyStage, float alpha) {
        ...
        if (tileEntityObject == null) {
            ...
            tileEntityObject.id.name = getBlockEntityId(tileEntity) + "( " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ() + ")";
